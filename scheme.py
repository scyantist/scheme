...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("Improper number of arguments")
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN Question 5A
        if self_evaluating(expressions.second.first):
            env.define(expressions.first, expressions.second.first)
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    runner = expressions
    while runner.second is not nil:
        if scheme_false(scheme_eval(runner.first, env)):
            return False
        runner = runner.second
    return scheme_eval(runner.first, env)
    # END Question 14B
...
def do_or_form(expressions, env):
    ...
    while runner is not nil:
        if scheme_true(scheme_eval(runner.first, env)):
            return scheme_eval(runner.first, env)
        runner = runner.second
    return False
    # END Question 14B
