...
def scheme_read(src):
    ...
    if val == "nil":
        ...
        #toReturn = Pair("quote", second)
        #while src.current() != "nil":
        #    second = Pair(scheme_read(src), scheme_read(src))
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN Question 2
            src.pop()
            element = scheme_read(src)
            if (src.pop() == ")"):
                return element
            else:
                raise SyntaxError("Not a valid dot notation")
            # END Question 2
        else:
